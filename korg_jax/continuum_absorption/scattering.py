...
def rayleigh_layers(nus, nH_I, nHe_I, nH2):
    """Batch Rayleigh scattering: density arrays are (n_layers,). Returns (n_layers, n_freq)."""
    nus   = jnp.asarray(nus)                             # (n_freq,)
    nH_I  = jnp.asarray(nH_I)[:, None]                  # (n_layers, 1)
    nHe_I = jnp.asarray(nHe_I)[:, None]
    nH2   = jnp.asarray(nH2)[:, None]
    ...
    alpha_H_He = nH_I * sigma_H + nHe_I * sigma_He      # (n_layers, n_freq)
