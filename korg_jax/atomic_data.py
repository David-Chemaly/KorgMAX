"""Element symbols, atomic masses, and solar abundance scales.

Transcribed from Korg.jl/src/atomic_data.jl.
"""
import numpy as np
from . import constants

# Element symbols (Z=1..92)
atomic_symbols = [
...
# symbol -> Z (1-based)
atomic_numbers = {s: i + 1 for i, s in enumerate(atomic_symbols)}

# Atomic masses in grams (multiply by amu)
_atomic_masses_amu = np.array([
...
atomic_masses = _atomic_masses_amu * constants.amu_cgs  # in grams

# ---------------------------------------------------------------------------
# Solar abundance scales  (A(X) = log10(N_X/N_H) + 12)
# ---------------------------------------------------------------------------
...
# Magg 2022: photospheric values override Bergemann 2025 where available
_magg_overrides = {
    "C": 8.56, "N": 7.98, "O": 8.77, "F": 4.40, "Ne": 8.15,
    "Na": 6.29, "Mg": 7.55, "Al": 6.43, "Si": 7.59, "P": 5.41,
    "S": 7.16, "Cl": 5.25, "Ar": 6.50, "K": 5.14, "Ca": 6.37,
    "Sc": 3.07, "Ti": 4.94, "V": 3.89, "Cr": 5.74, "Mn": 5.52,
    "Fe": 7.50, "Co": 4.95, "Ni": 6.24,
}
magg_2022_solar_abundances = np.copy(bergemann_2025_solar_abundances)
for sym, val in _magg_overrides.items():
    magg_2022_solar_abundances[atomic_numbers[sym] - 1] = val

# Default solar abundances (matches Julia Korg)
